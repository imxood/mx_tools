    ...
    ui(new Ui::MainWindow)
{
    ...
    //设置串口名
    m_device->setPortName("/dev/ttyUSB0");
...
{
    QByteArray buf = m_device->readAll();
    ...
    if(!buf.isEmpty()){
        QString text = tr(buf);
        qDebug() << text;
        setTextTermFormatting(ui->plainTextEdit, text);
    }
}


void MainWindow::setTextTermFormatting(QPlainTextEdit *textEdit, QString const &text)
{
    QTextDocument *document = textEdit->document();
    QRegExp const matcher(R"(\x1B\[([\d;]+)m)");// "\r\n\r\n\033[1;32muart:~$ \033[m"
    QTextCursor cursor(document);
    QTextCharFormat const defaultTextCharFormat = cursor.charFormat();
    cursor.beginEditBlock();
    int offset = matcher.indexIn(text);
    cursor.insertText(text.mid(0, offset));
    QTextCharFormat textCharFormat = defaultTextCharFormat;
    while (!(offset < 0)) {
        int previousOffset = offset + matcher.matchedLength();
        QStringList capturedTexts = matcher.capturedTexts().back().split(';');
        QListIterator<QString> i(capturedTexts);
        while (i.hasNext()) {
            bool ok = false;
            const QString str = i.next();
            qDebug() << str;
            int attribute = str.toInt(&ok);
            Q_ASSERT(ok);
            parseEscapeSequence(attribute, i, textCharFormat, defaultTextCharFormat);
        }
        offset = matcher.indexIn(text, previousOffset);
        if (offset < 0) {
